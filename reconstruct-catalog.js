...
function getTemplate(data) {
  return `import ProductSEO from "@/components/ProductSEO";
import { ArrowRight, Phone, Mail, ArrowLeft, CheckCircle, ShieldCheck, Zap, Waves, Rocket } from "lucide-react";
...
export default function ${data.functionName}() {
  const chemicalComposition = ${data.chemicalComposition};
  const equivalentGrades = ${data.equivalentGrades};
        ...
        <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
          <div className="container mx-auto px-4 py-4">
            <nav className="flex items-center space-x-2 text-sm flex-wrap gap-y-1">
              ...
              <ArrowRight className="w-4 h-4 text-gray-400" />
              <span className="text-navy-primary dark:text-gold-primary font-medium uppercase">${data.gradeDisplayName}</span>
            </nav>
          </div>
        </div>
          ...
          <div className="container mx-auto px-4 py-16 relative z-10">
            <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <h1 className="text-4xl md:text-6xl font-bold mb-4 uppercase tracking-tight">${data.gradeDisplayName} bars
                   <span className="block text-2xl text-gold-primary font-normal mt-2 normal-case">${data.subtitle}</span>
                </h1>
                <p className="text-xl mb-8 text-gray-200 leading-relaxed font-light">${data.descriptionSummary}</p>
              ...
              <div className="relative">
                <img src={ss_round_bar_mamta_steel_traders_img} alt="${data.gradeDisplayName} Round Bars" className="w-full rounded-2xl shadow-2xl border-4 border-white/10" loading="lazy" />
        ...
        <div className="container mx-auto px-4 py-12">
          <div className="max-w-7xl mx-auto">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 mb-12 border border-gray-100">
              <h2 className="text-2xl font-bold text-navy-primary dark:text-gray-100 mb-6 border-b-2 border-gold-primary pb-2 inline-block uppercase font-bold tracking-tighter italic">Grade Analysis: ${data.gradeDisplayName}</h2>
              <p className="text-gray-700 dark:text-gray-300 mb-6 text-lg leading-relaxed">${data.analysisText}</p>
            ...
            <div className="text-center bg-white dark:bg-gray-800 p-12 rounded-3xl shadow-2xl border border-gray-100 font-bold tracking-tighter italic">
              ...
              <div className="flex flex-col sm:flex-row gap-6 justify-center font-bold tracking-tighter italic">
                <Link href="/contact" className="bg-navy-primary hover:bg-navy-secondary text-white px-10 py-4 rounded-xl font-bold transition-all shadow-xl inline-flex items-center justify-center hover:scale-105 font-bold tracking-tighter italic">Connect with Exports</Link>
...
files.forEach(file => {
  try {
    ...
    // 1. Extract functionName
    const functionMatch = content.match(/export default function (\w+)/);
    const functionName = functionMatch ? functionMatch[1] : 'GradePage';

    // 2. Extract arrays (chemicalComposition, equivalentGrades, applications)
    const chemMatch = content.match(/const chemicalComposition\s*=\s*(\[[\s\S]*?\]);/);
    const equivMatch = content.match(/const equivalentGrades\s*=\s*(\[[\s\S]*?\]);/);
    const appMatch = content.match(/const applications\s*=\s*(\[[\s\S]*?\]);/);

    if (!chemMatch || !equivMatch || !appMatch) {
      console.warn(`Skipping ${file}: Data arrays not found`);
      return;
    }
    ...
    const canonMatch = content.match(/canonicalUrl="([\s\S]*?)"/);

    // 4. Extract H1 Grade Name
    const gradeDisplayMatch = content.match(/<h1[\s\S]*?>([\s\S]*?)bars/i);
    const gradeDisplayName = gradeDisplayMatch ? gradeDisplayMatch[1].trim() : 'Stainless Steel';

    const data = {
      functionName,
      chemicalComposition: chemMatch[1],
      equivalentGrades: equivMatch[1],
      applications: appMatch[1],
      title: titleMatch ? titleMatch[1] : `Stainless Steel Round Bars - Mamta Steel`,
      description: descMatch ? descMatch[1] : `Premium quality stainless steel round bars.`,
      keywords: kwMatch ? kwMatch[1] : `ss round bar, steel`,
      canonicalUrl: canonMatch ? canonMatch[1] : `/product/round-bars/stainless-steel`,
      gradeDisplayName,
      subtitle: 'Premium Precipitation Hardening / Corrosion Resistant Stock',
      descriptionSummary: 'Engineered for reliability in extreme conditions. Our precision-engineered round bars offer high fatigue life and superior structural integrity.',
      analysisText: 'This specialized grade is precisely engineered to balance high tensile strength with ductile performance, providing exceptional reliability in demanding mechanical environments.'
    };

    const newContent = getTemplate(data);
    fs.writeFileSync(filePath, newContent, 'utf8');
    console.log(`Reconstructed: ${file}`);
  } catch (err) {
    console.error(`Failed to reconstruct ${file}:`, err.message);
  }
});
